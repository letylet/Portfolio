      ...
      <RoomDecor id={id} > 
        {objects.map((obj, idx) => (
          <button
            ...
            className={`absolute group flex flex-col items-center justify-center transition-all duration-300 animate-pop-in hover:z-50 pointer-events-auto cursor-pointer ${themeStyles}`}
            style={{ 
              top: `${obj.y}%`, 
              left: `${obj.x}%`,
          ...
          >
            ...
            <span className="absolute -bottom-10 left-1/2 -translate-x-1/2 bg-black/80 text-white text-xs font-bold px-3 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-lg backdrop-blur-sm">
              {obj.label[language]}
            </span>
          </button>
        ))}
